<?php
	...
	class Upload_Helper {
		...
		private $_file_fix = '';
		//文件类型
		private $_file_type = array('image/png', 'image/gif', 'image/jpg', 'image/jpeg', 'application/octet-stream');
		//错误提示
		private $_error;
		//上传文件限制
		...
		public function __construct(array $vars){
			...
			$this->_file_ext = strtolower(substr($this->_file_name, strrpos($this->_file_name, '.') + 1));
		...
		private function mkRootDir(){
			if(!is_dir($this->_root_dir)){
				return @mkdir($this->_root_dir, 0777, true); //级联创建
			}else{
				return true;
			}
		}
		...
		private function mkFolder(){
			if($this->_is_mkdir){
				if(!is_dir($this->_folder_path)){
					return @mkdir($this->_folder_path, 0777, true);
				}else{
					return true;
				}
			}else{
		...
		public function getName(){
			return trim($this->_file_name);
		}
		public function getExt(){
			return strtolower(substr($this->_file_name, strrpos($this->_file_name, '.') + 1));
		}
		public function getSize(){
			return intval($this->_file_size * 1024);
		}

		//获取上传后 的文件路径
		public function getFilePath(){
			return trim($this->_file_new_path);
		}
